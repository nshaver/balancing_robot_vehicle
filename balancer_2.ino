...
int acc_calibration_value = -1740;                           //Enter the accelerometer calibration value (default=1000)

//Various settings
float pid_p_gain = 12;                                       //Gain setting for the P-controller (15)
float pid_i_gain = 2.1;                                      //Gain setting for the I-controller (1.5)
...
void setup(){
  ...
  pinMode(8, OUTPUT);                                                       //configure port for stepper enable
	digitalWrite(8, LOW);
  ...
  gyro_yaw_calibration_value /= 500;                                        //Divide the total value by 500 to get the avarage gyro offset
	sprintf(buf, "pitch=%d yaw=%d", gyro_pitch_calibration_value, gyro_yaw_calibration_value);
	Serial.println(buf);
...
void loop(){
  ...
  while ( radio.available() ) {
		...
		joystickX = map(joystick[0],0,1023,0,180); // turn value of 0-1023 to 0-180 degrees
		joystickY = map(joystick[1],0,1023,0,180);  // turn value of 0-1023 to 0-180 degrees
		
		// the four button variables from joystick array
		if (btn_A==0 && joystick[2]==1){
			// increase P gain
			pid_p_gain=pid_p_gain+1;
			Serial.print("pid_p_gain=");
			Serial.println(pid_p_gain);
		}
		btn_A = joystick[2];

		if (btn_B==0 && joystick[3]==1){
			// decrease I gain
			pid_i_gain=pid_i_gain-0.1;
			Serial.print("pid_i_gain=");
			Serial.println(pid_i_gain);
		}
		btn_B = joystick[3];

		if (btn_C==0 && joystick[4]==1){
			// decrease P gain
			pid_p_gain=pid_p_gain-1;
			Serial.print("pid_p_gain=");
			Serial.println(pid_p_gain);
		...
		if (btn_E==0 && joystick[6]==1){
			// increase D gain
			pid_d_gain=pid_d_gain+1;
			Serial.print("pid_d_gain=");
			Serial.println(pid_d_gain);
		}
		btn_E = joystick[6];

		if (btn_F==0 && joystick[7]==1){
			// decrease I gain
			pid_d_gain=pid_d_gain-1;
			Serial.print("pid_d_gain=");
			Serial.println(pid_d_gain);
	...
	/*
	// original serial receiver from nunchuk
  ...
  if(throttle_counter_left_motor > throttle_left_motor_memory){             //If the number of loops is larger then the throttle_left_motor_memory variable
    ...
    if(throttle_left_motor_memory < 0){                                     //If the throttle_left_motor_memory is negative
      ...
      throttle_left_motor_memory *= -1;                                     //Invert the throttle_left_motor_memory variable
			...
			//Set output 3 low to reverse the direction of the stepper controller
      PORTD &= 0b11011111;
			//digitalWrite(5, LOW);
		}
		...
		//Set output 2 low because the pulse only has to last for 20us 
		PORTD &= 0b11111011;
		//digitalWrite(2, LOW);
	}
  ...
  if(throttle_counter_right_motor > throttle_right_motor_memory){           //If the number of loops is larger then the throttle_right_motor_memory variable
    ...
    if(throttle_right_motor_memory < 0){                                    //If the throttle_right_motor_memory is negative
      ...
      throttle_right_motor_memory *= -1;                                    //Invert the throttle_right_motor_memory variable
			...
			//Set output 5 low to reverse the direction of the stepper controller
      PORTD |= 0b01000000;
			//digitalWrite(6, LOW);
		}
		...
		PORTD &= 0b11110111;            //Set output 4 low because the pulse only has to last for 20us
		//digitalWrite(3, LOW);
	}
}
